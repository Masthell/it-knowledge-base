...
- **Kali Linux VM**
  - Запущен **node_exporter** для сбора метрик системы
  - Метрики: CPU, RAM, диск, сеть
  - IP-адрес: `10.10.0.241`
  - Порт node_exporter: `9100`

- **Windows хост**
  - Запущен **Prometheus** (сбор и хранение метрик)
  - Запущена **Grafana** (визуализация метрик)
  - Работает основной проект
...
scrape_configs:
  ...
  - job_name: 'node'
    static_configs:
      - targets: ['10.10.0.241:9100']  # IP Kali VM с node_exporter

```

Проверка работы
Открыть Prometheus в браузере: http://localhost:9090/targets

Убедиться, что статус UP для:
localhost:9090 (Prometheus сам себя сканирует)
10.10.0.241:9100 (node_exporter на Kali VM)

## Настройка Grafana
1. Добавление источника данных
Войти в Grafana: http://localhost:3000
...
2. Импорт дашборда
Слева: "+ → Import"
Ввести ID дашборда: 1860
Нажать Load
Выбрать Data Source: Prometheus (http://localhost:9090)
Нажать Import
...
Сетевую активность
Если панели пустые:
Открыть любую панель в режиме редактирования
Проверить поле instance в Query
Заменить на 10.10.0.241:9100

### Проверить доступность метрик с Kali VM
curl http://10.10.0.241:9100/metrics

### Проверить статус в Prometheus UI
http://localhost:9090/targets
Проверка Grafana
Все графики должны отображаться корректно
Данные должны обновляться каждые 5 секунд
На панелях не должно быть ошибок "No data"

Проверка Kali Linux VM
Проверить статус node_exporter
systemctl status node_exporter
journalctl -u node_exporter -f
Проблема: Prometheus не видит node_exporter
Решение:

### Метрики
Доступные метрики через node_exporter
